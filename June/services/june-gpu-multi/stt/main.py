...
Multi-service version: runs on port 8001 in GPU container
"""
import asyncio
import logging
import uuid
...
logger = logging.getLogger("june-stt-multi")
...
async def join_livekit_room():
    ...
    await connect_room_as_subscriber(room, "june-stt-multi")
    room_connected = True
...
async def lifespan(app: FastAPI):
    logger.info("🚀 June STT (Multi-GPU Container)")
    try:
...
app = FastAPI(title="June STT Multi", version="6.1.0", description="Multi-GPU STT Service", lifespan=lifespan)
...
@app.get("/")
async def root():
    return {"service": "june-stt-multi", "version": "6.1.0", "pcm_pipeline": True, "sample_rate": SAMPLE_RATE}
