    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r.status_code == 429:
                ...
                logger.info(f"📤 Sent {'partial' if partial else 'final'} transcript to orchestrator: '{text}'")
...
async def _process_partial_transcript(pid: str, audio: np.ndarray, streamer: PartialTranscriptStreamer):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as tmp:
            ...
            # Avoid non-existent quick= param; use standard path
...
async def _process_utterances_with_streaming():
    global processed_utterances
    logger.info("🚀 Starting Silero VAD-enhanced STT processing")
...
async def lifespan(app: FastAPI):
    logger.info("🚀 June STT Enhanced - Silero VAD + LiveKit Integration + STREAMING")
    logger.info("Features: OpenAI API Compatible + Intelligent Speech Detection + Partial Transcripts")
...
async def root():
    return {
        ...
        "stats": {"processed_utterances": processed_utterances, **streaming_metrics.get_stats()},
    }
