...
class SessionManagerV2:
    ...
    def __init__(self):
        ...
        logger.info("✅ SessionManagerV2 initialized (backward-compatible wrapper)")
    
    def create_session(self, user_id: str, room_name: Optional[str] = None) -> 'SessionWrapper':
        """Create new session - backward compatible"""
        session = self._session_service._create_session(user_id, room_name)
    ...
    def get_or_create_session_for_room(self, room_name: str, user_id: str) -> 'SessionWrapper':
        ...
        session = self._session_service.get_or_create_for_room(room_name, user_id)
        return SessionWrapper(session)
    ...
    def get_session_by_room(self, room_name: str) -> Optional['SessionWrapper']:
        ...
        session = self._session_service.get_session_by_room(room_name)
        return SessionWrapper(session) if session else None
    
    def get_user_sessions(self, user_id: str) -> List['SessionWrapper']:
        """Get all sessions for a user"""
        sessions = self._session_service.get_user_sessions(user_id)
        return [SessionWrapper(session) for session in sessions]
    
    def delete_session(self, session_id: str) -> bool:
        """Delete session and cleanup mappings"""
        return self._session_service.delete_session(session_id)
    ...
    def add_to_history(
        ...
        self._session_service.add_message(session_id, role, content, metadata)
    ...
    def update_session_metrics(
        ...
        self._session_service.update_session_metrics(session_id, tokens_used, response_time_ms)
    
    def cleanup_expired_sessions(self, timeout_hours: int = 24) -> int:
        """Remove expired sessions"""
        return self._session_service.cleanup_expired_sessions(timeout_hours)
    
    def get_stats(self) -> Dict[str, Any]:
        """Get session manager statistics - backward compatible format"""
        stats = self._session_service.get_stats()
    ...
    def generate_guest_token(self, session_id: str, guest_name: str) -> Optional[str]:
        ...
        return self._session_service.generate_guest_token(session_id, guest_name)
...
class SessionWrapper:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        from .services.livekit_service import livekit_service  # Legacy import
...
# For Phase 1, we'll keep the original session_manager import working
# by creating an alias (this allows existing code to work unchanged)
