...
Correct ICE candidate handling using aiortc API
"""
import asyncio
import logging
...
from aiortc.sdp import candidate_from_sdp  # ✅ CRITICAL IMPORT
...
class PeerConnectionManager:
    ...
    FIXED: Proper ICE candidate handling
    """
        ...
        async def on_ice_candidate(candidate):
            ...
            if candidate:
                ...
                if self.websocket_manager:
                    try:
                        # aiortc candidates have a .candidate property that's pre-formatted
                        candidate_dict = {
    ...
    async def add_ice_candidate(self, session_id: str, candidate: dict):
        ...
        Args:
            ...
            candidate: Dict with 'candidate' (SDP string), 'sdpMid', 'sdpMLineIndex'
        """
        try:
            if session_id not in self.peers:
            ...
            if not candidate or not candidate.get("candidate"):
                ...
                # Signal end-of-candidates by passing None
                await pc.addIceCandidate(None)
                return
            
            # ✅ CRITICAL FIX: Parse the SDP candidate string using aiortc's helper
            ...
            if candidate_string.startswith("candidate:"):
                candidate_string = candidate_string[10:]  # Remove "candidate:" prefix
            
            # Parse using aiortc's built-in parser
            ice_candidate = candidate_from_sdp(candidate_string)
            
            # Set the sdpMid and sdpMLineIndex from the message
            ...
            logger.debug(f"[{session_id[:8]}]   Type: {ice_candidate.type}")
            logger.debug(f"[{session_id[:8]}]   Protocol: {ice_candidate.protocol}")
            logger.debug(f"[{session_id[:8]}]   Address: {ice_candidate.ip}:{ice_candidate.port}")
            
        except Exception as e:
            logger.error(f"[{session_id[:8]}] ❌ Error adding ICE candidate: {e}", exc_info=True)
            logger.error(f"[{session_id[:8]}]   Candidate data: {candidate}")
