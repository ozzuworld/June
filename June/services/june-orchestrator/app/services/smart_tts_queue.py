#!/usr/bin/env python3
"""
Smart TTS Queue - Natural Conversation Flow with Parallel Synthesis
Reduces latency by pre-synthesizing upcoming phrases while current phrase plays
"""
import asyncio
import logging
...
@dataclass
class TTSPhrase:
    """Represents a TTS phrase with priority, timing info, and synthesis state"""
    text: str
    room_name: str
...
class SmartTTSQueue:
    """
    Intelligent TTS queue with parallel synthesis for reduced latency
    Synthesizes upcoming phrases while current phrase is playing
    """
    def __init__(self, tts_service, max_concurrent: int = 2, phrase_gap_ms: int = 50):
        self.tts_service = tts_service
        self.max_concurrent = max_concurrent  # Allow 2+ parallel syntheses
        ...
        logger.info(f"🎵 SmartTTSQueue initialized: max_concurrent={max_concurrent}, phrase_gap={phrase_gap_ms}ms, parallel_synthesis=ENABLED")
    ...
    async def queue_phrase(
        ...
        try:
            if not text or len(text.strip()) == 0:
                return False
            # Initialize session locks if needed
            ...
            if is_first_phrase:
                ...
                # Start synthesis AND playback immediately for first phrase
                ...
                # Start synthesis immediately (parallel), playback waits for order
                phrase.synthesis_task = asyncio.create_task(self._synthesize_phrase(phrase))
                asyncio.create_task(self._process_phrase_sequential(phrase, session_id))
                logger.info(f"📋 QUEUED TTS: '{text[:30]}...' for {session_id} (synthesis started)")
            return True
    ...
    async def _synthesize_phrase(self, phrase: TTSPhrase) -> bool:
        """
        Synthesize audio for a phrase (can run in parallel with other syntheses)
        Sets audio_ready event when complete
        """
        try:
            async with self.synthesis_semaphore:
                logger.info(f"🔊 Synthesizing (parallel): '{phrase.text[:30]}...'")
                ...
                if success:
                    ...
                    phrase.audio_ready.set()  # Set anyway to unblock playback
                    return False
        except Exception as e:
    ...
    async def _process_phrase_urgent(self, phrase: TTSPhrase):
        """Process urgent (first) phrase - no wait, immediate playback when ready"""
    ...
    async def _process_phrase_sequential(self, phrase: TTSPhrase, session_id: str):
        """
        Process sequential phrase - wait for previous phrase playback to finish,
        but synthesis runs in parallel in the background
        """
        try:
            # Use playback lock to ensure phrases play in order
            async with self.session_playback_locks[session_id]:
                # Small gap between phrases for natural timing
                await asyncio.sleep(self.phrase_gap_ms / 1000.0)
                # Wait for this phrase's synthesis to complete (may already be done!)
                await phrase.audio_ready.wait()
                self.metrics["phrases_processed"] += 1
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            "queue_type": "parallel_synthesis_smart_queue"
        }
...
def initialize_smart_tts_queue(tts_service, max_concurrent: int = 2, phrase_gap_ms: int = 50):
    ...
    logger.info("🎵 Global SmartTTSQueue initialized with parallel synthesis")
