    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"📊 TTS result: {response.json()}")
            else:
...
async def publish_tts_to_room(request: TTSPublishRequest):
    ...
    return {"status": "success", "room_name": request.room_name, "text_length": len(request.text)}


@router.get("/api/sessions/stats")
async def get_sessions_stats():
    return {"status": "success", "stats": session_manager.get_stats()}
...
async def clear_session_history(session_id: str):
    ...
    return {"status": "success", "session_id": session_id, "cleared_messages": old_count, "message": "Conversation history cleared"}
