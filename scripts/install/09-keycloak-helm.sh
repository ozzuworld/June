...
helm upgrade --install keycloak bitnami/keycloak \
  ...
  --set production=true \
  --set proxy=edge \
  --set-string extraEnvVars[0].name=KC_HOSTNAME \
  --set-string extraEnvVars[0].value="$KEYCLOAK_HOSTNAME" \
  --set-string extraEnvVars[1].name=KC_HOSTNAME_STRICT \
  --set-string extraEnvVars[1].value=false \
  --set-string extraEnvVars[2].name=KC_HTTP_RELATIVE_PATH \
  --set-string extraEnvVars[2].value=/ \
