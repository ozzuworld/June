...
POD_NAME=$(kubectl get pod -n media-stack -l app=jellyseerr -o jsonpath='{.items[0].metadata.name}')
...
kubectl cp "$(dirname "$0")/setup-jellyseerr-wizard.py" \
    "media-stack/$POD_NAME:/tmp/setup-jellyseerr.py"

# Copy API key files into the pod
echo "[INFO] Copying API keys to pod..."
kubectl cp /root/.radarr-api-key "media-stack/$POD_NAME:/tmp/radarr-api-key" 2>/dev/null || echo "[WARN] Radarr API key not found"
kubectl cp /root/.sonarr-api-key "media-stack/$POD_NAME:/tmp/sonarr-api-key" 2>/dev/null || echo "[WARN] Sonarr API key not found"

# Install Python requests library if not present
echo "[INFO] Ensuring Python dependencies are installed..."
kubectl exec -n media-stack "$POD_NAME" -- sh -c "pip install requests 2>/dev/null || apk add py3-requests 2>/dev/null || apt-get update && apt-get install -y python3-requests 2>/dev/null || true"

# Run the setup script from inside the pod
echo "[INFO] Running Jellyseerr setup from inside pod..."
kubectl exec -n media-stack "$POD_NAME" -- python3 /tmp/setup-jellyseerr.py \
    --url "http://localhost:5055" \
    --domain "$DOMAIN" \
    --jellyfin-user "$JELLYFIN_USER" \
    --jellyfin-pass "$JELLYFIN_PASS" \
    --jellyfin-url "http://jellyfin.media-stack.svc.cluster.local:8096"
