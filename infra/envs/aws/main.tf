...
module "kube" {
  source       = "../../providers/aws/kubernetes_cluster"
  name         = var.name
  region       = var.region
...
module "registry" {
  source = "../../providers/aws/container_registry"
  name   = var.name
  region = var.region
}

module "dns" {
  source    = "../../providers/aws/dns_zone"
...
module "bucket" {
  source = "../../providers/aws/object_storage"
  name   = "${var.name}-artifacts"
  region = var.region
}

module "kms" {
  source = "../../providers/aws/secrets_kms"
  name   = var.name
  region = var.region
}
