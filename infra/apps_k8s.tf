...
resource "kubernetes_deployment" "orch" {
  metadata { name = "june-orchestrator"; namespace = local.apps_ns; labels = { app = "june-orchestrator" } }
  spec {
    replicas = 2
    selector { match_labels = { app = "june-orchestrator" } }
    template {
      metadata { labels = { app = "june-orchestrator" } }
      spec {
        ...
        container {
          ...
          liveness_probe  { tcp_socket { port = local.default_port } initial_delay_seconds = 20 period_seconds = 10 }
          readiness_probe { tcp_socket { port = local.default_port } initial_delay_seconds = 10 period_seconds = 5 }
...
resource "kubernetes_service" "orch" {
  ...
  spec {
    ...
    port { name = "http"; port = 80; target_port = local.default_port }
    type = "ClusterIP"
  }
}
    ...
    rule {
      ...
      http {
        path {
          path      = "/"
          path_type = "Prefix"
          backend { service { name = kubernetes_service.orch.metadata[0].name; port { number = 80 } } }
        }
      }
    }
  }
...
resource "kubernetes_deployment" "stt" {
  metadata { name = "june-stt"; namespace = local.apps_ns; labels = { app = "june-stt" } }
  spec {
    replicas = 2
    selector { match_labels = { app = "june-stt" } }
    template {
      metadata { labels = { app = "june-stt" } }
      spec {
...
resource "kubernetes_service" "stt" {
  ...
  spec {
    ...
    port { name = "http"; port = local.default_port; target_port = local.default_port }
    type = "ClusterIP"
  }
}
...
resource "kubernetes_deployment" "tts" {
  metadata { name = "june-tts"; namespace = local.apps_ns; labels = { app = "june-tts" } }
  spec {
    replicas = 2
    selector { match_labels = { app = "june-tts" } }
    template {
      metadata { labels = { app = "june-tts" } }
      spec {
